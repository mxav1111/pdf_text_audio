...
while curr_position_in_utf < utf_buffer_size:
    ...
    while text_chunk_size < 2000:
        ...
        if (
            ...
            text_chunk = (
                text_chunk + file_in_buffer[start_of_sentence:]
        ...
        print(
            f"start_of_sentence={start_of_sentence},end_of_sentence={end_of_sentence},text_chunk_size={text_chunk_size},len={len(text_chunk)}, curr_position_in_utf={curr_position_in_utf}, utf_buffer_size={utf_buffer_size}"
        )
    print(
        f'file_in_buffer details:- encodeutflen={len(file_in_buffer.encode("utf-8"))},len={len(file_in_buffer)}, " WHOLEFILE" '
    )
    print(f"{text_chunk}")
    ...
    audio_chunk_filename = (
        file_to_write + "_" + str(file_sequence_to_write).zfill(3) + ".mp3"
    )
    text_chunk_filename = (
        file_to_write + "_" + str(file_sequence_to_write).zfill(3) + ".out"
    )
    ...
    print(
        f"Audio and text content written to file :- {audio_chunk_filename} {text_chunk_filename}"
    )
    collect_audio_chunk_filenames = (
        collect_audio_chunk_filenames + audio_chunk_filename + "|"
    )
...
if os.path.isfile(file_to_writeext):
    ...
    if removepartial == "YES":
        ...
        print(
            f"Partial files not removed because environment variable removepartial is set to {removepartial}"
        )
else:
    print(
        f"{file_to_writeext} could not be created. You might have partial files {file_to_write}*.out and {file_to_write}*.mp3 available for further diagnosis."
    )
