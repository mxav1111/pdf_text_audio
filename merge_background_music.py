...
background_audio_filename=os.environ['bga'] ; voicename=os.environ['voicename']        ; speakingrate=os.environ['speakingrate'] 
...
audio_filename=os.environ['book']+sep+voicename+sep+speakingrate; removepartial=os.environ['removepartial']
credential_path = os.environ['credential_path']; bktnm=os.environ['bktnm']
location = os.environ['location'] ; project_id = os.environ['project_id'] ; output_gcs_uri = "gs://"+bktnm+"/"+audio_filename
...
exec_os_command='ffmpeg -hide_banner -y -i '+voice_audio_filename+' -stream_loop -1 -i '+background_audio_filename+' -lavfi "[1:a]volume=0.1,apad[A];[0:a][A]amerge[out]" -map [out]:a -shortest -b:a 192k '+audio_filenameext
print(f"Merging {voice_audio_filename} and {background_audio_filename} : {audio_filenameext} \n using: {exec_os_command}"); 
